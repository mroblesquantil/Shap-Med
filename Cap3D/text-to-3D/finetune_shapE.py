...
class shapE_train_dataset(Dataset):
    def __init__(self, latent_code_path):
        self.captions = pd.read_csv('./example_material/human_livers.csv', header=None)
        self.valid_uid = list(pickle.load(open('./example_material/training_human_liver.pkl','rb')))
...
class shapE_val_dataset(Dataset):
    def __init__(self, latent_code_path):
        self.captions = pd.read_csv('./example_material/human_livers.csv', header=None)
        self.valid_uid = list(pickle.load(open('./example_material/validation_human_liver.pkl','rb')))
        self.final_uid = self.valid_uid
        self.n2idx = {}
        for i in range(len(self.captions)):
            self.n2idx[self.captions[0][i]] = i
